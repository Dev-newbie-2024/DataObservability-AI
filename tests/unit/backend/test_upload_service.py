...
class TestCleanColName:
    ...
    def test_special_chars_replaced(self):
        assert _clean_col_name("Price ($)") == "price___"  # $ and () → _
...
class TestProcessUpload:
    ...
    def test_new_dataset_upload_succeeds(self, mock_sf, mock_kafka):
        from backend.services.upload_service import process_upload

        # Configure mocks
        mock_sf.get_dataset_by_name.return_value = None  # new dataset
        mock_sf.insert_dataset.return_value = "ds-uuid-001"
        mock_sf.insert_pipeline_run.return_value = "run-uuid-001"
        mock_sf.bulk_insert_bronze.return_value = 3
        mock_sf.get_current_schema_version.return_value = None
        ...
        mock_sf.create_bronze_table.assert_called_once()
        mock_sf.insert_schema_version.assert_called_once()
    ...
    @patch("backend.services.upload_service.snowflake_client")
    def test_kafka_failure_does_not_block_upload(self, mock_sf, mock_kafka):
        """Kafka errors should be swallowed — upload still succeeds."""
        from backend.services.upload_service import process_upload

        mock_sf.get_dataset_by_name.return_value = None
        mock_sf.insert_dataset.return_value = "ds-uuid-003"
        mock_sf.insert_pipeline_run.return_value = "run-uuid-003"
        mock_sf.bulk_insert_bronze.return_value = 3
        mock_sf.get_current_schema_version.return_value = None
